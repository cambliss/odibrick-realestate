...
const PropertyGridCard = ({ property, onRemove }) => (
  <motion.div
    ...
    className="bg-white rounded-2xl border border-[#E2E8F0] shadow-card hover:shadow-card-hover overflow-hidden group transition-shadow duration-300"
  >
    {/* Image */}
    <div className="relative h-48 overflow-hidden bg-[#F8FAFC]">
      {property.image?.[0] ? (
        ...
        <div className="w-full h-full flex items-center justify-center">
          <Building2 className="w-12 h-12 text-[#E2E8F0]" />
        </div>
      )}
      {/* Badges */}
      <div className="absolute top-3 left-3 flex gap-1.5">
        <span className="px-2.5 py-1 bg-[#1C1B1A]/80 backdrop-blur-sm text-[#FFFFFF] text-xs font-semibold rounded-full">
          {property.type}
        </span>
        <span className={cn(
    ...
    <div className="p-4">
      ...
      <div className="flex items-center gap-3 text-xs text-[#5A5856] mb-4">
        <span className="flex items-center gap-1"><BedDouble className="w-3.5 h-3.5 text-[#004AAD]" />{property.beds} Beds</span>
        <span className="flex items-center gap-1"><Bath className="w-3.5 h-3.5 text-[#004AAD]" />{property.baths} Baths</span>
        <span className="flex items-center gap-1"><Maximize className="w-3.5 h-3.5 text-[#004AAD]" />{property.sqft} sqft</span>
      </div>

      {/* Price + Actions */}
      <div className="flex items-center justify-between pt-3 border-t border-[#F8FAFC]">
        <div>
          <span className="text-lg font-bold text-[#004AAD]">{formatPrice(property.price)}</span>
          {property.availability === "rent" && <span className="text-xs text-[#9CA3AF] ml-1">/mo</span>}
        </div>
        <div className="flex items-center gap-1.5">
          <Link to={`/update/${property._id}`}>
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              className="p-2 bg-[#FFFFFF] border border-[#E2E8F0] text-[#5A5856] rounded-lg hover:border-[#004AAD] hover:text-[#004AAD] transition-all duration-200">
              <Edit3 className="w-4 h-4" />
            </motion.button>
          </Link>
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
            onClick={() => onRemove(property._id, property.title)}
            className="p-2 bg-[#FFFFFF] border border-[#E2E8F0] text-[#5A5856] rounded-lg hover:border-red-300 hover:text-red-500 hover:bg-red-50 transition-all duration-200">
...
const PropertyListRow = ({ property, onRemove }) => (
  <motion.div
    ...
    exit={{ opacity: 0, x: -10 }}
    className="flex items-center gap-4 p-4 bg-white rounded-xl border border-[#E2E8F0] hover:border-[#004AAD]/30 hover:shadow-card transition-all duration-200"
  >
    {/* Thumbnail */}
    <div className="w-16 h-16 rounded-xl overflow-hidden bg-[#F8FAFC] flex-shrink-0">
      {property.image?.[0] ? (
    ...
    <div className="flex-1 min-w-0">
      <div className="flex items-center gap-2 mb-0.5">
        ...
        <span className="px-2 py-0.5 bg-[#1C1B1A] text-[#FFFFFF] text-xs rounded-full flex-shrink-0">{property.type}</span>
      </div>
    ...
    <div className="hidden md:flex items-center gap-4 text-xs text-[#5A5856]">
      <span className="flex items-center gap-1"><BedDouble className="w-3.5 h-3.5 text-[#004AAD]" />{property.beds}</span>
      <span className="flex items-center gap-1"><Bath className="w-3.5 h-3.5 text-[#004AAD]" />{property.baths}</span>
      <span className="flex items-center gap-1"><Maximize className="w-3.5 h-3.5 text-[#004AAD]" />{property.sqft}</span>
    </div>

    {/* Price */}
    <div className="text-right flex-shrink-0">
      <div className="font-bold text-[#004AAD] text-sm">{formatPrice(property.price)}</div>
      <div className={cn(
    ...
    <div className="flex items-center gap-1.5 flex-shrink-0">
      <Link to={`/update/${property._id}`}>
        <button className="p-2 text-[#5A5856] hover:text-[#004AAD] hover:bg-[#004AAD]/10 rounded-lg transition-all duration-200">
          <Edit3 className="w-4 h-4" />
        </button>
      </Link>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div>
            ...
            <p className="text-[#5A5856] text-sm">
              <span className="font-semibold text-[#004AAD]">{filteredProperties.length}</span> listings found
            </p>
          </div>
          <div className="flex items-center gap-2">
            <motion.button onClick={handleRefresh} disabled={refreshing}
              whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              className="flex items-center gap-2 px-4 py-2.5 bg-white border border-[#E2E8F0] text-[#1C1B1A] rounded-xl text-sm font-medium hover:border-[#004AAD] hover:text-[#004AAD] transition-all shadow-card disabled:opacity-60">
              <RefreshCw className={cn("w-4 h-4", refreshing && "animate-spin")} />
              <span className="hidden sm:inline">Refresh</span>
            </motion.button>
            <Link to="/add">
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                className="flex items-center gap-2 px-5 py-2.5 bg-[#004AAD] hover:bg-[#003B8B] text-white rounded-xl text-sm font-semibold transition-all shadow-lg hover:shadow-[#004AAD]/25">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            ...
            {/* Type Filter */}
            <div className="flex items-center gap-1 bg-[#FFFFFF] rounded-xl p-1 flex-shrink-0">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)}
                className="px-3 py-2.5 bg-[#FFFFFF] border border-[#E2E8F0] rounded-xl text-xs text-[#5A5856] outline-none focus:border-[#004AAD] transition-all cursor-pointer">
                {SORT_OPTIONS.map((opt) => (
                  <option key={opt.value} value={opt.value}>{opt.label}</option>
                ))}
              </select>

              <div className="flex items-center bg-[#FFFFFF] border border-[#E2E8F0] rounded-xl p-1">
        ...
        <AnimatePresence mode="wait">
          {filteredProperties.length === 0 ? (
            <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="text-center py-20 bg-white rounded-2xl border border-[#E2E8F0]">
              <div className="w-16 h-16 bg-[#F8FAFC] rounded-2xl flex items-center justify-center mx-auto mb-4">
                <Home className="w-8 h-8 text-[#E2E8F0]" />
              </div>
              ...
              {!searchTerm && filterType === "all" && (
                <Link to="/add">
                  <button className="px-6 py-3 bg-[#004AAD] text-white rounded-xl font-semibold text-sm hover:bg-[#003B8B] transition-colors">
                    Add First Property
                  </button>
                </Link>
              )}
