...
class EmailService {
  constructor() {
    this.fromAddress = process.env.EMAIL_USER || 'noreply@odibrick.com';
  }
  ...
  async sendWelcomeEmail(userEmail, userName) {
    const subject = 'Welcome to Odibrick - Your Account is Ready!';
  ...
  async sendEmailVerification(userEmail, userName, verificationUrl) {
    const subject = 'Verify Your Odibrick Email Address';
  ...
  async sendPasswordResetEmail(userEmail, resetUrl) {
    const subject = 'Reset Your Odibrick Password';
  ...
  async sendNewsletterWelcome(userEmail) {
    const subject = 'Welcome to Odibrick Newsletter';
    const siteUrl = process.env.WEBSITE_URL || 'https://odibrick.vercel.app';
  ...
  async sendAppointmentNotificationToAdmin(appointment, userDetails) {
    ...
    const adminEmail = process.env.ADMIN_EMAIL || 'admin@odibrick.com';
  ...
  async sendUserSuspended(userEmail, userName, days, reason, suspendedUntil) {
    const subject = `Odibrick Account Suspended - ${days} Day${days > 1 ? 's' : ''}`;
  ...
  async sendUserBanned(userEmail, userName, reason) {
    const subject = 'Odibrick Account Permanently Banned';
  ...
  async sendUserReactivated(userEmail, userName) {
    const subject = 'Odibrick Account Reactivated - Welcome Back!';
