...
export function useSEO({ title, description, image, url, type = 'website' }: SEOProps) {
  useEffect(() => {
    const fullTitle = title ? `${title} | Odibrick` : DEFAULT_TITLE;
    ...
    return () => {
      ...
      setOG('og:title', 'Odibrick - AI-Powered Luxury Real Estate');
      setOG('og:description', DEFAULT_DESCRIPTION);
      setOG('og:image', DEFAULT_IMAGE);
      setOG('og:url', SITE_URL);
      setOG('og:type', 'website');
      setTwitter('twitter:title', 'Odibrick - AI-Powered Luxury Real Estate');
