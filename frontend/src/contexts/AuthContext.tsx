...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [token, setToken] = useState<string | null>(localStorage.getItem('odibrick_token'));
  const [isLoading, setIsLoading] = useState(true);

  // On mount, check if token exists and is valid
  useEffect(() => {
    const storedToken = localStorage.getItem('odibrick_token');
    const storedUser = localStorage.getItem('odibrick_user');
  ...
  const login = useCallback(async (email: string, password: string, rememberMe: boolean = false) => {
    ...
    if (data.success && data.token) {
      ...
      localStorage.setItem('odibrick_user', JSON.stringify(data.user));
      setToken(data.token);
      setUser(data.user);
    } else {
  ...
  const updateUser = useCallback((partial: Partial<User>) => {
    setUser((prev) => {
      ...
      localStorage.setItem('odibrick_user', JSON.stringify(next));
      return next;
    });
  }, []);
